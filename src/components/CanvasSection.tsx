  ...
  async function fetchCanvasCode() {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          prompt:`
Create a HTML canvas code to visualize demonstration of "${topic}".

Use these brief examples for context:

Example 1 (OS Architecture):
- Visualize key OS components (e.g., Hardware, CPU, Memory, Storage, I/O, Software, UI) as labeled rectangles.
- Draw connecting lines between components to show relationships.
- Ensure the canvas is sized appropriately (e.g., 800x600) and styled with a clean, modern look.

Example 2 (A* Algorithm):
- Draw a grid (e.g., 20x20 cells).
- Mark start and goal nodes in distinct colors.
- Randomly generate walls and animate the pathfinding process (show open set, closed set, and final path).
- Use simple shapes and colors to represent nodes and connections.

Example 3 (Stacks & Queues):
- Visualize a stack as blue boxes (LIFO) and a queue as green boxes (FIFO) on the canvas.
- Provide buttons or controls for push/pop (for the stack) and enqueue/dequeue (for the queue).
- Ensure the elements are drawn with proper spacing and updated on each operation.

Your generated code should be concise, fit within the specified canvas dimensions, and use modern HTML5 canvas techniques. Do not exceed the word limit.`,
        }),
      });
